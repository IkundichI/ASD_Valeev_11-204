public class TwoThreeTree {
    ...
    //Класс Node от которого наследуются ThreeNode и LeafNode
    private class Node {

    }

    //Класс ThreeNode отвечает за структуру узла дерева
    private class TreeNode extends Node {

        int keys[];             //Ключи для поиска
        Node children[];        //Ссылки на детей этого родителя
        int degree;             //Количество дочерних элементов
    ...
    //Класс LeadNode отвечает за листья
    private class LeafNode extends Node {
        int key;    //Для сохранения значений
    ...
    //Это рекурсивный метод вставки элемента, который будет использоватья для public метода
    ...
    //Это рекурсивный метод для удаления Node из Tree
    ...
    //Этот метод обновляет информацию, хранящуюся в ThreeNode
    ...
    //Этот метод возвращает значение для ключей TreeNode
    ...
    //Этот метод выполняет поиск по заданному ключу в дереве
    ...
    //Этот публичный метод нужен для вызова рекурсивного (метода для вставки элемента)
    ...
    //Это публичный метод для вызова search (для поиска элемента в дереве)
    public boolean search(int key) {
        countOfIteration = 0;
        return search(key, root);
    }

    //Это публичный метод для удаления элемента из дерева (вызывает привытный)
